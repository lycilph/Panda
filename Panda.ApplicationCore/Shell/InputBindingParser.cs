...
namespace Panda.ApplicationCore.Shell
{
    public static class InputBindingParser
    {
        ...
        public static TriggerBase CreateTrigger(string trigger_text)
        {
            ...
            var modifier_keys = all_keys.Take(all_keys.Count() - 1).ToList();
            if (modifier_keys.Any())
                modifiers = modifier_keys.Select(s => (ModifierKeys) Enum.Parse(typeof (ModifierKeys), s))
                                         .Aggregate((current, mod) => current | mod);

            var key_binding = new KeyBinding(new InputBindingTrigger(), key, modifiers);
